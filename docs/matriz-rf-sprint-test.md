...
| RF-06 | Procesamiento multivariable e índice de riesgo | 4, 8.5B | **RF-06E: Implementado V1 corregido y enriquecido** | `POST …/procesar-riesgo`, `MlRiskService` | UI procesar riesgo **activada V1** (NC-11 cerrada V1); botón en perfil estudiante con permiso `procesar_riesgo`; smoke manual navegador aprobado con observaciones menores | `RiesgoTest` (38 passed, 125 assert.) + `SemaforoCompletitudTest` (11) + `HistorialRiesgoTest` (12) = 61 tests, 210 assert. | **RF-06E cerrado V1** | ML **determinístico**; sin VSE ni Fast Test; mínimo: notas+asistencia; conducta opcional; payload enriquecido (nota_min, cursos_riesgo/desap, inasistencias, recientes, graves, gravedad, reincidencia); pesos 55/30/15; Flask validado 5 escenarios. Nivel Inicial sin cálculo de riesgo en V1. ML real (RF-18) pendiente |
...
| RF-11 | Atención psicológica perfil integral | 5, 8, RF-11B–E | **Implementado V1 con smoke manual aprobado con observaciones menores** | Permiso `ver_perfil_psicologo_tutor` (RF-11B); endpoint `GET /api/psicologo-tutor/seguimiento` + controller + tests (RF-11C); frontend `PerfilPsicologoTutorPanel.jsx` + menú lateral (RF-11D); cierre documental y validaciones RF-11E | Menú lateral **Seguimiento psicólogo/tutor** | `PsicologoTutorSeguimientoTest` 20 passed (RF-11C); regresión RF-06/RF-14/RF-16/RF-19/RF-20 OK; build/lint OK | RF-11E cerrada V1; smoke manual navegador aprobado con observaciones menores. No es funcionalidad clínica. Docente y directivo fuera de V1 |
...
| RF-14 | Dashboard académico e institucional | 6A, 6B, 7A, RF-14E | **Implementado V1 con smoke manual aprobado con observaciones menores** | `GET /api/dashboard` existente (riesgo subset). Backend `GET /api/dashboard/institucional` (RF-14C) + UI `DashboardInstitucionalPanel.jsx` (RF-14D) implementados; permiso `ver_dashboard_institucional`. `ver_dashboard` se mantiene | Menú lateral **Dashboard institucional** | `DashboardTest` 12 passed (76 assertions); `DashboardInstitucionalTest` **16 passed** (57 assertions); regresión RF-06/RF-16/RF-19/RF-20 (74 tests, 246 assertions); build frontend OK; lint: 88 problemas preexistentes, `DashboardInstitucionalPanel.jsx` limpio | RF-14E cerrado V1; smoke manual navegador aprobado con observaciones menores | RF-14 V1 cerrado: permiso, backend y frontend institucional implementados. Sin PDF/exportación nuevos. Sin gráficos complejos. Sin selector de sede. Dashboard legacy conservado |
| RF-15 | Gestión usuarios y control acceso | 2, 8 | Confirmado en código | `/api/usuarios`, Spatie (**30 permisos** en seeder) | **Usuarios** (admin) | `GestionUsuariosTest`, `AuthenticationTest`; Cypress auth/logout | Parcial 401; Cypress 2H: 2 auth tests passed, login/logout E2E pendiente por credenciales | [`seguridad-roles-permisos.md`](seguridad-roles-permisos.md) |
| RF-16 | Generación reportes de riesgo académico | 6B, RF-16E | **Implementado V1 con smoke manual aprobado con observaciones menores** | Backend: `GET /api/reportes/riesgo-academico` paginado + filtros + `ReporteRiesgoAcademicoController`. Frontend: `ReporteRiesgoAcademicoPanel.jsx` + `App.jsx` menú "Reportes de riesgo". Permiso `ver_reportes_riesgo` | Menú lateral **Reportes de riesgo** | `ReporteRiesgoAcademicoTest` 13 passed (36 assertions); regresión RF-06/RF-19/RF-20 (61 tests); build frontend OK; lint preexistente 88 problemas | RF-16E cerrado V1; smoke manual navegador aprobado con observaciones menores | Backend+frontend+permiso implementados. PDF/exportación (`generar_reportes_riesgo`) fuera de V1. No recalcula riesgo ni llama Flask |
...
| RF-20 | Historial riesgo evolutivo | 4, 6A | **Implementado V1** | Tabla `indices_riesgo`; permiso `ver_historial_riesgo`; endpoint `GET /api/estudiantes/{estudiante}/historial-riesgo`; componente `EstudiantePerfilHistorialRiesgo.jsx` | Perfil estudiante → **Historial de riesgo académico** | `HistorialRiesgoTest.php` 12 passed, 30 assertions; build frontend OK | Fases 4B–4E | Backend + frontend V1 implementados; **smoke manual navegador aprobado con observaciones menores**; Cypress global no ejecutado |
...
3. Marcar como **planificado**: RF-10, RF-18. **RF-04:** implementado V1 mínimo (Fase 2E). **RF-06:** RF-06E cerrado V1; smoke manual navegador aprobado con observaciones menores; nivel Inicial sin cálculo de riesgo en V1. **RF-11:** **implementado V1** (RF-11B/C/D/E: permiso, backend, frontend, cierre documental); smoke manual navegador aprobado con observaciones menores; Cypress no ejecutado. **RF-14:** implementado V1; smoke manual navegador aprobado con observaciones menores. **RF-16:** **implementado V1** (backend + frontend + tests); smoke manual navegador aprobado con observaciones menores. **RF-19:** implementado V1 (Fases 3B–3E); smoke manual navegador aprobado; Cypress no ejecutado. **RF-20:** **implementado V1** (Fases 4B–4E: RBAC, backend, frontend y cierre documental); smoke manual navegador aprobado con observaciones menores; Cypress global no ejecutado.
