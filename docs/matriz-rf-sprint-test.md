...
| RF-11 | Atención psicológica perfil integral | 5, 8, RF-11B–D | **Implementado V1 con smoke manual pendiente** | Permiso `ver_perfil_psicologo_tutor` (RF-11B); endpoint `GET /api/psicologo-tutor/seguimiento` + controller + tests (RF-11C); frontend `PerfilPsicologoTutorPanel.jsx` + menú lateral (RF-11D) | Menú lateral **Seguimiento psicólogo/tutor** | `PsicologoTutorSeguimientoTest` 20 passed (RF-11C) | RF-11D build OK | RF-11B/C/D cerradas V1; RF-11E (smoke manual + cierre documental) pendiente. No es funcionalidad clínica. Docente y directivo fuera de V1 |
