...
| RF-16 | Generación reportes de riesgo académico | 6B, RF-16B | **Parcial / en avance** | Permiso `ver_reportes_riesgo` implementado en seeder (RF-16B) | Botón export dashboard (PDF parcial) | `DashboardTest`, `ActivityLogTest` | No re-ejecutado Fase 5 | Base RBAC implementada (RF-16B). Pendientes: endpoint, controller, frontend, tests y cierre. PDF/exportación fuera de V1 |
