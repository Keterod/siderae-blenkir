...
| RF-16 | Generación reportes de riesgo académico | 6B, RF-16D | **Parcial / en avance** | Backend: `GET /api/reportes/riesgo-academico` paginado + filtros + `ReporteRiesgoAcademicoController`. Frontend: `ReporteRiesgoAcademicoPanel.jsx` + `App.jsx` menú "Reportes de riesgo". Permiso `ver_reportes_riesgo` | Menú lateral **Reportes de riesgo** | `ReporteRiesgoAcademicoTest` 13 passed; build frontend OK | Ejecutado RF-16D | Backend+frontend implementados. Smoke manual pendiente. PDF/exportación fuera de V1. No recalcula riesgo ni llama Flask |
