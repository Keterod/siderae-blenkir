...
| RF-16 | Generación reportes de riesgo académico | 6B, RF-16C | **Parcial / en avance** | `GET /api/reportes/riesgo-academico` paginado + filtros + `ReporteRiesgoAcademicoController`. Permiso `ver_reportes_riesgo` | Sin UI todavía | `ReporteRiesgoAcademicoTest` 13 passed | Ejecutado RF-16C | Backend implementado. Frontend pendiente. PDF/exportación fuera de V1. No recalcula riesgo ni llama Flask |
