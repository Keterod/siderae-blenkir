...
| RF-14 | Dashboard académico e institucional | 6A, 6B, 7A, RF-14D | **Parcial / en avance** | `GET /api/dashboard` existente (riesgo subset). Backend `GET /api/dashboard/institucional` (RF-14C) + UI `DashboardInstitucionalPanel.jsx` (RF-14D) implementados; permiso `ver_dashboard_institucional`. `ver_dashboard` se mantiene | Menú lateral **Dashboard institucional** | `DashboardTest` 12 passed (76 assertions); `DashboardInstitucionalTest` **16 passed** (57 assertions); build frontend OK; lint: 88 problemas preexistentes, nuevo componente limpio | RF-14D ejecutado | Backend+frontend institucional implementados. Pendiente: smoke manual y cierre RF-14E. Sin PDF/exportación nuevos. Sin selector de sede |
