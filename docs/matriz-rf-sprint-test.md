...
| RF-14 | Dashboard académico e institucional | 6A, 6B, 7A, RF-14C | **Parcial / en avance** | `GET /api/dashboard` existente (riesgo subset). Backend `GET /api/dashboard/institucional` implementado (RF-14C) con permiso `ver_dashboard_institucional`. `ver_dashboard` se mantiene | **Dashboard** (riesgo subset) | `DashboardTest` 12 passed (76 assertions); `DashboardInstitucionalTest` **16 passed** (57 assertions) | RF-14C ejecutado | Backend institucional implementado. Pendiente: UI institucional, tests extendidos, smoke manual. Sin PDF/exportación nuevos. Sin selector de sede |
