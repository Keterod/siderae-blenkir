...
| NC-11 | *(Hallazgo histórico)* UI riesgo **en pausa**; sin botón procesar riesgo — **corregido y cerrado V1** | [`manual-usuario.md`](../manual-usuario.md), `EstudiantePerfilRiesgo.jsx` | RF-06/20 parcial en UX | Media | Backend RF-06E completo (38 passed, 125 assertions). UI de procesamiento activada: botón **Procesar/Actualizar riesgo** en perfil estudiante con permiso `procesar_riesgo`; llama `POST /api/estudiantes/{id}/procesar-riesgo`; refresca historial RF-20 y semáforo RF-19. No recalcula automáticamente al abrir perfil. No usa VSE ni Fast Test. Smoke manual navegador aprobado con observaciones menores | Frontend + doc | **Cerrada V1** (NC-11); smoke manual navegador aprobado con observaciones menores |
...
| NC-18 | RF-16 reportes de riesgo | DRS v2.1 | PDF dashboard parcial | Media | Backend `GET /api/reportes/riesgo-academico`, frontend `ReporteRiesgoAcademicoPanel.jsx`, permiso `ver_reportes_riesgo`, docs y tests (13 passed). Smoke manual navegador aprobado con observaciones menores; PDF/exportación (`generar_reportes_riesgo`) fuera de V1 | Backend + Frontend + doc | **Cerrada V1** (RF-16E); smoke manual navegador aprobado con observaciones menores |
| NC-22 | RF-14 dashboard académico e institucional | DRS v2.1 §RF-14 | Dashboard legacy era solo subset de riesgo; faltaba vista institucional ampliada | Media | Backend `GET /api/dashboard/institucional` (`DashboardInstitucionalController`), frontend `DashboardInstitucionalPanel.jsx`, permiso `ver_dashboard_institucional` asignado a admin/coord/directivo, docs y tests (16 passed). Smoke manual navegador aprobado con observaciones menores; sin PDF/exportación/gráficos complejos; sede Chilca; docente y psicólogo/tutor fuera de V1 | Backend + Frontend + doc | **Cerrada V1** (RF-14E); smoke manual navegador aprobado con observaciones menores |
...
| NC-20 | RF-20 historial evolutivo | DRS v2.1 | Timeline por periodo | Media | UI perfil riesgo | Frontend | **Cerrada V1** (Fases 4B–4E); smoke manual navegador aprobado con observaciones menores; Cypress global no ejecutado |
...
| NC-23 | RF-11 seguimiento psicólogo/tutor | DRS v2.1 §RF-11 | Panel de seguimiento tutorial no existía | Media | Permiso `ver_perfil_psicologo_tutor`, endpoint `GET /api/psicologo-tutor/seguimiento`, controller `PsicologoTutorSeguimientoController`, tests 20 passed, frontend `PerfilPsicologoTutorPanel.jsx`, helper API, menú lateral e icono. Sin diagnóstico clínico, sin historia médica, sin PDF/exportación, sin recalcular riesgo, sin Flask, sin VSE/Fast Test, sede Chilca; docente y directivo fuera de V1 | Backend + Frontend + doc | **Cerrada V1** (RF-11E); smoke manual navegador aprobado con observaciones menores |
